    ...
    stages {
        ...
        stage("Start Docker") {
            steps {
                script {
                    dir("${env.WORKSPACE}") {
                        sh 'docker-compose -f compose.jenkins.yml down --remove-orphans -v'  // Ensure containers and volumes are removed
                        ...
                        // ok
                        sh 'docker compose -f compose.jenkins.yml up -d --build'
                        sh 'docker compose -f compose.jenkins.yml ps'
                    }
                }
            }
        }
